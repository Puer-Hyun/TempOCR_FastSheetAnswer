...
# CORS 설정
allowed_origins = [
    "http://10.178.0.7",    # App-Server 운영
    "http://10.178.0.11",   # App-Server 스테이징
    "http://10.178.0.4",    # ASPX 서버 운영
    "http://10.178.0.12",   # ASPX 서버 스테이징
    "http://112.175.72.197",  # ASPX 서버 스테이징 (HTTPS)
    "https://10.178.0.7",   # App-Server 운영 (HTTPS)
    "https://10.178.0.11",  # App-Server 스테이징 (HTTPS)
    "https://10.178.0.4",   # ASPX 서버 운영 (HTTPS)
    "https://10.178.0.12",  # ASPX 서버 스테이징 (HTTPS)
    "https://112.175.72.197",  # ASPX 서버 스테이징 (HTTPS)
    "https://starlauncher.postdemy.com",
    "http://starlauncher.postdemy.com"
]
...
1. 정답 추출 규칙:
   ...
   - 새끼 문제가 있다- 라는 것의 정의는 정답에 "12. (1) ABC (2) DEF" 라고 적혀있는 것이라서, number가 12-(1), 12-(2) 로 구분하여 새끼문제가 있는거야.
   - "12. (ㄱ) ABC (ㄴ) EDF (ㄷ) AXC" 같은 경우는 새끼문제가 있는 것이 아니라, 하나의 문제에 대한 답이 "(ㄱ) ABC (ㄴ) EDF (ㄷ) AXC" 이거 인거야.

2. 응답 형식:
   - 각 이미지들을 전부 참조하여 정답을 추출할 것.
   - JSON 형식으로 응답
   - number는 반드시 숫자로 시작해야하고, 최대 2단계까지만 진행해야함. I.삼각형의 성질 등의 서술이 있더라도, 24-(1), 24-(2)와 같이 최대 단계수는 2단계로 대답할 것.
   - 즉즉
   - 예시: {
       "status": "success", 
       "answers": [
...
3. 주의사항:
   ...
   - score가 없다면 score는 0 으로 처리해야합니다.
   - 서술형이란 문장으로 이루어져있거나, 정답 자체가 '표' 이거나 '그림' 이어서 Latex 등으로 표현할 수 없는 것을 서술형 문제라고 함. 
"""
